...
namespace IronXna
{
	...
	public static class SpriteBatchPrimitiveExtensions
	{
		...
		/// </summary>
		public static void DrawLine(this SpriteBatch spriteBatch, Vector2 p, Vector2 p1, Color color)
		{
			float distance = Vector2.Distance(p, p1);
			float angle = (float)Math.Atan2(p1.Y - p.Y, p1.X - p.X);

			spriteBatch.Draw(_pixel, p, null, color, angle, Vector2.Zero, new Vector2(distance, 1), SpriteEffects.None, 0);
		}
		/*
